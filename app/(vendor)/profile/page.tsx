          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div className="relative z-10 w-full max-w-md rounded-xl border bg-background p-5 text-foreground shadow-xl">
              ...
              <div className="grid gap-3">
                <input aria-label="Store Name" className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={storeForm.name} onChange={(e) => setStoreForm({ ...storeForm, name: e.target.value })} placeholder="Store name" />
                <input aria-label="Address" className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={storeForm.address} onChange={(e) => setStoreForm({ ...storeForm, address: e.target.value })} placeholder="Address (optional)" />
                <div className="pt-1 flex justify-end gap-2">
                  <button className="border rounded-md px-3 py-2 text-xs" onClick={() => setShowDialog(false)}>Cancel</button>
                  <button className="rounded-md bg-primary px-3 py-2 text-xs text-primary-foreground" onClick={async () => {
                    if (!email || !storeForm.name.trim()) return;
                    const res = await fetch('/api/store/create', {
                      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, name: storeForm.name.trim(), address: storeForm.address.trim() || undefined })
                    });
                    if (res.ok) {
                      const data = await res.json();
                      setVendor((v: any) => ({ ...(v || {}), store: data.store }));
                      window.dispatchEvent(new Event('vendor:updated'));
                      setToast({ show: true, message: 'Store setup successful' });
                      setShowDialog(false);
                    }
                  }}>Save</button>
                </div>
