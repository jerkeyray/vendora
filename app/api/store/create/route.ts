...
export async function POST(request: Request) {
  try {
    ...
    const existing = await prisma.store.findUnique({ where: { vendorId: vendor.id } });
    if (existing) return NextResponse.json({ store: existing }, { status: 200 });

    const store = await prisma.store.create({ data: { vendorId: vendor.id, name, address: address || null } });
